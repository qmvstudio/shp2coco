...
class GeoTiff(object):
        ...
        for i in range(row_num):
            for j in range(col_num):
                ...
                for id in range(len(ins_list)):
                    ...
                    try:
                        save_path = os.path.join(out_dir, '%d_%s_%d.tif' % (begin_id+i*col_num+j, 'building', id))
                        # print(bg_img, gtiffDriver,self.dataset, j*clip_size, i*clip_size, save_path)
    ...
    def mask_tif_with_shapefile(self, shapefile_path, label=255):
        ...
        print("mask_tif_with_shapefile finished!")
    ...
    def clip_tif_and_shapefile(self, clip_size, begin_id, shapefile_path, out_dir):
        ...
        pic_id = self.clip_tif_with_grid(clip_size=clip_size, begin_id=begin_id, out_dir=out_dir + '/building_2021')
        print("pic_id",pic_id)
        return pic_id
